define(["core/js/adapt", "./adapt-assessmentQuestionBank"], function (
  Adapt,
  QuestionBank
) {
  var givenIdCount = 0;
  ...
  var AssessmentModel = {
    ...
    init: function () {
      ...
      this._currentQuestionComponents = this.findDescendantModels(
        "components",
        { where: { _isQuestionType: true } }
      );
      this.set(
        "_currentQuestionComponentIds",
        this._currentQuestionComponents.map(function (comp) {
          return comp.get("_id");
        })
      );
    ...
    _setupAssessmentData: function (force, callback) {
      ...
      var shouldResetAssessment =
        (!this.get("_attemptInProgress") && !state.isPass) || force === true;
      var shouldResetQuestions =
        (assessmentConfig._isResetOnRevisit &&
          (state.allowResetIfPassed || !state.isPass)) ||
        force === true;
      ...
      if (shouldResetAssessment) {
        ...
        if (
          ...
          assessmentConfig._randomisation &&
    ...
    _setupBankedAssessment: function () {
      ...
      for (var bankId in this._questionBanks) {
        if (this._questionBanks.hasOwnProperty(bankId)) {
          // skip over properties that were added to Array.prototype by the ES5-shim for IE8
    ...
    _setupRandomisedAssessment: function () {
      ...
      if (randomisationModel._blockCount > 0) {
        questionModels = questionModels.slice(
          0,
          randomisationModel._blockCount
        );
      }
    ...
    _overrideQuestionComponentSettings: function () {
      ...
      if (!_.isEmpty(newSettings)) {
        for (
          var i = 0, l = this._currentQuestionComponents.length;
          i < l;
          i++
        ) {
    ...
    _checkAssessmentComplete: function () {
      ...
      var allQuestionsAnswered =
        numberOfQuestionsAnswered >= this._currentQuestionComponents.length;
        ...
        var questionModel = {
          ...
          _isCorrect:
            questionComponent.get("_isCorrect") === undefined
              ? null
              : questionComponent.get("_isCorrect"),
        };
    ...
    _checkIsPass: function () {
      ...
      var isPass = isPercentageBased
        ? scoreAsPercent >= scoreToPass
        : score >= scoreToPass;
    ...
    _getMarkingSettings: function () {
      ...
      if (this._shouldSuppressMarking()) {
        ...
        if (questionConfig.hasOwnProperty("_canShowModelAnswer")) {
          markingSettings._canShowModelAnswer =
            questionConfig._canShowModelAnswer;
        }
    ...
    _resetQuestions: function (callback) {
      ...
      function step() {
        for (
          var j = 0, count = Math.min(syncIterations, len - i);
          j < count;
          i++, j++
        ) {
          var question = qs[i];
    ...
    isAssessmentEnabled: function () {
      if (this.get("_assessment") && this.get("_assessment")._isEnabled)
        return true;
      return false;
    },
    ...
    reset: function (force, callback) {
      ...
      if (
        this.get("_assessmentCompleteInSession") &&
        !assessmentConfig._isResetOnRevisit &&
        !isPageReload &&
        !force
      ) {
    ...
    getSaveState: function () {
      ...
      var banksActive =
        cfg._banks && cfg._banks._isEnabled && cfg._banks._split.length > 1;
      var randomisationActive =
        cfg._randomisation && cfg._randomisation._isEnabled;
      ...
      if (!banksActive && !randomisationActive) {
        ...
        this.findDescendantModels("components").forEach(function (component) {
          var componentModel = {
            ...
            _isCorrect:
              component.get("_isCorrect") === undefined
                ? null
                : component.get("_isCorrect"),
          };
    ...
    setRestoreState: function (restoreState) {
      ...
      this.set({
        ...
        _attemptsLeft:
          attempts === "infinite" ? attempts : attempts - attemptsSpent,
      ...
      for (id in indexByIdQuestions) {
        if (
          indexByIdQuestions.hasOwnProperty(id) &&
          Adapt.findById(id).get("_isQuestionType")
        ) {
          questions.push({
            _id: id,
    ...
    getState: function () {
      ...
      var state = {
        ...
        shouldPretestCompleteCourse:
          Adapt.course.attributes._assessment._shouldPretestCompleteCourse,
        ...
        questionModels: new Backbone.Collection(
    ...
    getConfig: function () {
      ...
      if (!assessmentConfig) {
        ...
        assessmentConfig = $.extend(
          true,
          {},
          assessmentConfigDefaults,
          assessmentConfig
        );
      }
