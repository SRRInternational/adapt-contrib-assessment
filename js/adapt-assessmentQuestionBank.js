define(function (require) {
  ...
  QuestionBank.prototype = {
      ...
      for (var i = 0; i < this._numQuestionBlocks; i++) {
        ...
        if (question !== undefined) {
          ...
          var index = Math.floor(
            Math.random() * (usedQuestionBlocks.length - 1)
          );
    ...
    checkResetUnunsedBlocks: function () {
      if (this.unUsedQuestionBlocks !== undefined && this._uniqueQuestions)
        return;
    ...
    getRandomQuestion: function () {
      if (
        this.unUsedQuestionBlocks !== undefined &&
        this.unUsedQuestionBlocks.length < 1
      ) {
        console.warn(
          "assessment:" +
            this._articleId +
            " No more unique questions for _assessment._quizBankID " +
            this._id
        );
        return undefined;
      }
      ...
      var index = Math.round(
        Math.random() * (this.unUsedQuestionBlocks.length - 1)
      );
