  ...
  Adapt.assessment = _.extend(
    {
      ...
      _onAssessmentsComplete: function (state) {
        ...
        if (!this._getStateByAssessmentId(assessmentId)) {
          Adapt.log.warn(
            "assessments: state was not registered when assessment was created"
          );
        }
      ...
      _setLockedPageProgress: function (questions) {
        ...
        _.forEach(questionsCollection, function (question) {
          ...
          for (g = 0; g < question.length; g++)
            questionIsCorrect.push(question[g]._isCorrect);
          for (h = 0; h < questionIsCorrect.length; h++) {
            //check if any of them is false
            //if false mark page unlock false
            if (questionIsCorrect[h] === false)
              questionIsCorrect.unlockPage = false;
      ...
      _markPageComplete: function (state) {
        ...
        if (questionIsCorrect.length === question.length) {
          ...
          if (unlockPage) {
            if (state.shouldPretestCompleteCourse)
              this._unlockPagesByModel(Adapt.contentObjects.models);
            else {
              var unlockingPages = [];
              let pageModel = [];
              _.forEach(Adapt.blocks.models, function (pageModel) {
                ...
                if (
                  state.preTestPageId === parentPageModel.attributes._parentId
                ) {
      ...
      _handleRoute: function (plugin, id) {
        if (
          (plugin !== "assessment" && plugin !== "article-assessment") ||
          id === undefined
        ) {
          return;
        }
      ...
      _checkResetAssessmentsOnRevisit: function (toObject) {
        ...
        var pageAssessmentModels = this._getAssessmentByPageId(
          toObject._currentId
        );
        ...
        Adapt.wait.for(function resetAllAssessments(allAssessmentHaveReset) {
          ...
          pageAssessmentModels.forEach(function (model) {
            model.reset(forceAssessmentReset, function () {
              ...
              var haveAllModelsReset =
                numberOfResetAssessments === numberOfAssessments;
      ...
      _setPageProgress: function () {
        ...
        for (var k in this._assessments._byPageId) {
          ...
          for (var i = 0, l = assessments.length; i < l; i++) {
            ...
            if (assessmentState.includeInTotalScore && !assessmentState.isPass)
              continue;
      ...
      _addToAssessmentIdMap: function (id, model) {
        if (id === undefined) {
          Adapt.log.warn(
            'An assessment has been registered with an undefined value for "_id"'
          );
          return;
        }
        ...
        if (id === "") {
          Adapt.log.warn(
            'An assessment has been registered with an empty value for "_id"'
          );
        }
        ...
        if (!this._assessments._byAssessmentId[id]) {
          ...
          Adapt.log.warn(
            'An assessment with an _id of "' + id + '" already exists!'
          );
        }
      },
      ...
      saveState: function () {
        ...
        for (
          var i = 0, assessmentModel;
          (assessmentModel = this._assessments[i++]);
      ...
      getConfig: function () {
        ...
        if (assessmentsConfig === undefined) {
          ...
          assessmentsConfig = $.extend(
            true,
            {},
            assessmentsConfigDefaults,
            assessmentsConfig
          );
        }
